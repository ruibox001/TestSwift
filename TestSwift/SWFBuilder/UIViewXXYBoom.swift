...
extension UIView {
    ...
    @objc private func cellAnimations(){
        for shape in boomCells!{
            ...
            moveAnimation.path = makeRandomPath(aLayer: shape).cgPath
            ...
            //ios10 opacityAnimation.delegate = false
    ...
    private func makeScaleValue() -> CGFloat{
        return 0.7;// 1 - 0.7 * (CGFloat(arc4random()%101 - 50)/CGFloat(50))
    }
    ...
    private func makeRandomPath(aLayer:CALayer) -> UIBezierPath{
        ...
        let temp = UInt32(1.2 * layer.frame.size.height);
        
        let controlPointOffSetY = layer.position.y - 0.2 * layer.frame.size.height - CGFloat(arc4random()%temp)
        let endPointY = layer.position.y + layer.frame.size.height/2 + CGFloat(arc4random()%UInt32(layer.frame.size.height/2))
        
        particlePath.addQuadCurve(to: makePoint(endPointX, endPointY), controlPoint: makePoint(controlPointOffSetX, controlPointOffSetY))
        return particlePath
    }
    
    private func colorWithPoint(x:Int,y:Int,image:UIImage) -> UIColor{
        
        let pixelData = image.cgImage?.dataProvider
...
//        layer.renderInContext(UIGraphicsGetCurrentContext()!)
    ...
    func boom(){
        ...
        _ = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(UIView.scaleOpacityAnimations), userInfo: nil, repeats: false)
            ...
            for i in 0...16{
                for j in 0...16{
                    ...
                    if scaleSnapshot == nil{
                        ...
                        scaleSnapshot = snapshot().scaleImageToSize(size: makeSize(34, 34))
                    }
                    let pWidth = min(frame.size.width,frame.size.height)/17
                    let color = UIColor.white; //scaleSnapshot!.getPixelColorAtLocation(CGPointMake(CGFloat(i * 2), CGFloat(j * 2)))
                    let shape = CALayer()
                    shape.backgroundColor = color.cgColor
                    shape.opacity = 0
                    shape.cornerRadius = pWidth/2
                    shape.frame = makeRect(CGFloat(i) * pWidth, CGFloat(j) * pWidth, pWidth, pWidth)
//                    shape.frame = CGRectMake(CGFloat(i) * pWidth, CGFloat(j) * pWidth, pWidth, pWidth)
        ...
        _ = Timer.scheduledTimer(timeInterval: 0.2, target: self, selector: #selector(UIView.cellAnimations), userInfo: nil, repeats: false)
...
extension UIImage{
    ...
    private var aRGBBitmapContext:CGContext?{
        get{
...
//            let bitmapData = UnsafeMutablePointer<Void>.alloc(bitmapByteCount)
            ...
            let context = CGContext.init(data: bitmapData, width: pixelsWidth, height: pixelsHeitht, bitsPerComponent: 8, bytesPerRow: bitmapBytesPerRow, space: colorSpace, bitmapInfo: CGBitmapInfo.byteOrderMask.rawValue | CGImageAlphaInfo.premultipliedFirst.rawValue)
//            let context = CGBitmapContextCreate(bitmapData,pixelsWidth,pixelsHeitht,8,bitmapBytesPerRow,colorSpace, CGBitmapInfo.ByteOrderDefault.rawValue | CGImageAlphaInfo.PremultipliedFirst.rawValue)!
    ...
    func scaleImageToSize(size:CGSize) -> UIImage{
        ...
        draw(in: makeRect(0, 0, size.width, size.height))
